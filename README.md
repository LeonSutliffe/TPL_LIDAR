...
## Setting up from scratch (Windows/WSL2 — the currently-working path)

This is the exact sequence used on the dev machine, reconstructed from
its actual install history (not reconstructed from memory/guesswork —
e.g. the `micromamba create`/`install` lines below are pulled straight
from `conda-meta/history` in the live environment). It's the
currently-validated path; the Pi deployment below is still in progress.
...
```
Reboot if `wsl --install` asks for one (first-time WSL2 install often
does). `dorssel.usbipd-win` is what lets Windows hand a USB device
(the RS485 bridge) through to WSL2.

**2. Force WSL2 into mirrored networking mode**

Without this, `usbipd attach` and the GUI's `ws://localhost:9090`
connection both break (see `HANDOFF.md`'s "Known gotchas" for the
failure mode). Create `%USERPROFILE%\.wslconfig`:
```ini
[wsl2]
...
wsl --shutdown
```
(next `wsl` launch picks up the new config).

**3. Install micromamba inside WSL2 and build the `ros2` environment**

```bash
mkdir -p ~/micromamba-bin
curl -Ls https://micro.mamba.pm/api/micromamba/linux-64/latest | tar -xvj -C ~/micromamba-bin --strip-components=1 bin/micromamba

~/micromamba-bin/micromamba create -r ~/micromamba -n ros2 -y \
    -c robostack-jazzy -c conda-forge \
    ros-jazzy-desktop ros-jazzy-rosbridge-suite ros-jazzy-velodyne ros-jazzy-xacro \
    rosdep vcstool pyserial \
    colcon-common-extensions compilers cmake pkg-config make ninja git
```
(One consolidated command here — the real history split this into six
separate `create`/`install` calls made incrementally as each package
turned out to be needed; the end result is identical.) No `micromamba
shell init` is used anywhere in this project — every invocation passes
`-r ~/micromamba -n ros2` (or uses `micromamba run`) explicitly instead,
so nothing here depends on shell hooks being set up.

**4. Clone the repo**

The workspace lives on the **Windows filesystem** and is built/run from
WSL2 via its `/mnt/` path — not a WSL-native clone. Matches how this
project has always been built and run:
```powershell
cd D:\Downloads
git clone https://github.com/LeonSutliffe/TPL_LIDAR.git LIDAR
```
...
From inside WSL2:
```bash
cd /mnt/d/Downloads/LIDAR/ros2_ws
~/micromamba-bin/micromamba run -r ~/micromamba -n ros2 bash -c "source ~/micromamba/envs/ros2/setup.bash && colcon build"
```
(`source .../envs/ros2/setup.bash` first activates the ROS2 distro
itself, distinct from this workspace's own `install/setup.bash` sourced
at launch time in step 7 — the two are not interchangeable.)

**6. Connect the hardware, bind the USB bridge for passthrough**

Plug in the VLP-16 (Ethernet) and the USB↔RS485 bridge. Binding is a
**one-time** step per device (survives replugs, does not survive a
Windows USB stack reset) and needs an **elevated** PowerShell/Command
Prompt:
```powershell
usbipd list
```
Find the RS485 bridge's `BUSID` (an FTDI FT232 device — `0403:6001` —
if using the same adapter this project currently ships with), then:
```powershell
usbipd bind --busid <busid-from-list>
```

**7. Launch**

Easiest: double-click `scripts\start_scanner.ps1` (or a desktop shortcut
to it, if you make one) — it wakes WSL2, finds the bridge by VID:PID and
attaches it, launches the full stack in its own window, and opens the
GUI automatically. Manual equivalent, once the bridge is attached
(`usbipd attach --wsl --busid <busid>`, from an elevated prompt, needed
again after every `wsl --shutdown` or extended WSL2 idle — attach itself
is not one-time like bind):
```bash
cd /mnt/d/Downloads/LIDAR/ros2_ws
~/micromamba-bin/micromamba run -r ~/micromamba -n ros2 bash -c "source install/setup.bash && ros2 launch scanner_bringup bringup.launch.py"
```
Then open `web/tilt_axis_gui/index.html` in a browser and connect to
`ws://localhost:9090`.

See `HANDOFF.md`'s "Running it" and "Known gotchas" sections for launch
args, troubleshooting, and the full incident history behind several of
the steps above.
