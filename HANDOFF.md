...
**Changed (2026-09-20), per explicit request: scans now save as binary PCD, not E57**, because the E57 writer (Python, per-packet encoding) made saves take minutes (see the e57_writer perf to-do). `pcd_writer.write_pcd` writes a text header plus raw float32 rows (x, y, z, intensity, ring, time) in ~8MB chunks with real progress and fsync -- bounded by SD write speed. Round-trip verified locally against `read_pcd_points` (bit-exact). The Scans list, project-bundle matching and both GUIs' project-name regex accept `.pcd` alongside `.e57`; legacy `.e57` files still list/download. Trade-off: E57 metadata (mount calibration provenance, station name) is no longer embedded in new scans. Deployed to the Pi (`colcon build --packages-select scan_aggregator`, `tpl-scanner.service` restarted, active) but **not yet verified with a real scan** -- save time and file size still to be measured. Project bundling deliberately left as-is for now (per request).

**Changed (2026-09-20), per explicit request: saved PCDs now hold x, y, z, intensity only** (ring and time dropped, 24 -> 16 bytes/point, ~33% smaller; no point density lost). Confirmed after the PCD switch above that file size had not changed vs E57 (both stored the same six float32 fields), only save time (500MB in <10s vs ~4 min). `write_pcd` slices columns per chunk so no extra full-array copy is made. Local round-trip verified; deployed to the Pi.
